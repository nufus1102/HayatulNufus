...
if($statusUpload == 0){
    ...
    if(mysqli_num_rows($select) > 0){
        ...
        if(move_uploaded_file($_FILES['foto']['tmp_name'],$target_file)){
            $query = mysqli_query($conn, "INSERT INTO tb_daftar_menu (foto,nama_menu,keterangan,harga,stok) 
            values ('" .$kode_rand. $_FILES['foto']['name']."','$nama_menu','$keterangan','$harga','$stok')");
    if($query){
